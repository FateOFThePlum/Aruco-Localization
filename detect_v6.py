...
#Settings For Logan
Source = 'camera' #'file' or 'camera'
bluring = False
binary_Method = 'otsu' #'threshhold' or 'otsu'
dual_photo_mode = False
baud_rate = 9600
...
def insize(size): 
    ...
    wantedSize = 1
    if size < wantedSize+tolerance and size > wantedSize - tolerance:
        return True
    else:
        return False
...
def BigPortionV1 (img, frame):
    ...
    # QR code detection
    if binary_Method == 'threshhold':
        qr_codes = pyzbar.decode(cv2.medianBlur(frame, setting_ksize))
        #frame = cv2.resize(frame, (0, 0), fx = 0.25, fy = 0.25)
        #img = cv2.resize(img, (0, 0), fx = 0.25, fy = 0.25)
        maskImg = img
    elif binary_Method == 'otsu':
        #does a GausianBlur then does an OTSU binarization
        blured_og_img = cv2.cvtColor(og_img, cv2.COLOR_BGR2GRAY)
        blured_og_img = cv2.GaussianBlur(blured_og_img,(5,5),0)
        ret,blured_og_img = cv2.threshold(blured_og_img,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        qr_codes = pyzbar.decode(blured_og_img)
        printer("Done Decoding")
        cv2.imshow("f", cv2.resize(blured_og_img, (0, 0), fx=0.25, fy=0.25))
        maskImg = img
    ...
    if qr_code_count > 0:
        ...
        average_pixel_to_inch_ratio = perimeter_sum / (qr_code_count * actual_inch_length * 0.5)
...
def measurement_img_Auto_Exposure_False(maskImg, img, average_pixel_to_inch_ratio):
    ...
    if dual_photo_mode == True and Source == 'camera': 
        printer('Sending On Command')
        turnLightOn()
        printer('Sleeping')
        time.sleep(2)
        ret, MaskImg = video_capture.read()
...
def app_port(og_img):
    global bluring
    global qr_codes_detected
    global img, object_height, object_width, qr_codes_detected 
    printer('You Are Useing The App Port')
    qr_codes_detected = False
    ...
    if bluring:
        ...
        printer('Nuh UH')
        img, object_height, object_width, qr_codes_detected = BigPortionV1(og_img, og_img)
        cv2.imshow('IMG', img)

    try:
        return object_width, object_height, qr_data
    except:
        return 0, 0, 0
        ...
        try:
            ...
            while True:
                ...
                if keyCode == ord('w'):
                    ...
                    og_img = cv2.rotate(og_img, cv2.ROTATE_180)
                    object_width, object_height, qr_data = app_port(og_img)
                    ...
                    ret, og_img = video_capture.read()
                    og_img = cv2.resize(og_img, None, fx = 0.5, fy = 0.5)
                    og_img = cv2.rotate(og_img, cv2.ROTATE_180)
                ...
                if cv2.getWindowProperty(window_title, cv2.WND_PROP_AUTOSIZE) >= 0:
                    cv2.imshow(window_title, og_img)
                else:
                    break
                keyCode = cv2.waitKey(10) & 0xFF
                # Stop the program on the ESC key or 'q'
...
turnLightOff()
if Source == 'camera' or Source == 'cam' or Source == 'live':
    video_capture = cv2.VideoCapture(gstreamer_pipeline(), cv2.CAP_GSTREAMER)
